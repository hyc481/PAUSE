...
def load_branch_tool_allowlist(branch_name: str) -> List[str] | None:
    """
    Load branch tool allowlist from `bench.pipeline.generate_task.branches.<branch_name>`.
    Convention:
    ...
    """
    try:
        mod = importlib.import_module(f"bench.pipeline.generate_task.branches.{branch_name}")
    except Exception:
        return None
