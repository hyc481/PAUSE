...
from bench.utils.paths import RUNS_DIR
from bench.pipeline.build_store import load_store_from_meta_profile
from bench.utils.agent_runner import AgentRunner
from bench.utils.clients import get_traj_model_routes
from bench.utils.misc import load_branch_tool_allowlist, load_branch_assistant_guidance, group_routes_by_key
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--tasks_file",
        default=str(RUNS_DIR / "developing" / "tasks" / "tasks_developing_20260113_090755.json"),
    )
    parser.add_argument(
        "--tasks_files",
        default=str(RUNS_DIR / "testrun5" / "tasks"),
        help="If provided, run all task files under this directory (recursive), ignoring --tasks_file.",
    )
    parser.add_argument(
        "--output_dir",
        default=str(RUNS_DIR / "testrun5" / "inference_logs"),
        help="If provided, output all generated trajectories under this directory (recursive).",
    )
